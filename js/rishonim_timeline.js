...
d3.csv('data/Rishonim.csv').then((data) => {
  ...
  data.sort(function (a, b) {
    return (
      d3.descending(a['birth'], b['birth']) ||
      d3.descending(a['death'], b['death'])
    );
  });
  ...
  const xAxis = d3
    .scaleLinear()
    .domain([d3.min(data, (d) => d['birth']), d3.max(data, (d) => d['death'])])
    .range([0, WIDTH]);
  svg
    .append('g')
  ...
  const cScale = d3.scaleOrdinal().domain(locations).range(colors);
